...
class EducationInterventionWidget extends BaseWidget
{
    ...
    public function table(Table $table): Table
    {
        return $table
            ->query(
                Orphan::query()
                    ->whereHas('interventionRequests', fn (Builder $q) => $q->whereHas('type', fn ($q) => $q->where('name', 'like', '%education%'))
                    )
                    ->withCount(['interventionRequests as education_requests' => fn (Builder $q) => $q->whereHas('type', fn ($q) => $q->where('name', 'like', '%education%')),
                    ])
            ...
            ->columns([
                ...
                TextColumn::make('educations.support_amount')
                    ...
                    ->formatStateUsing(fn ($record) => number_format($record->educations->first()?->support_amount ?? 0, 2)
                    ),
