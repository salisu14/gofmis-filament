{{-- resources/views/id-cards/bulk-template.blade.php --}}
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <style>
        ...
        * {
        ...
        .header {
            border-bottom: 0.2mm solid {{ $accentColor ?? '#1E90FF' }};
            padding-bottom: 1mm;
            margin-bottom: 1.2mm;
            height: 15.5mm;
        }
        ...
        .security-hologram {
            ...
            width: 8mm;
            height: 8mm;
            background: linear-gradient(45deg, rgba(255,255,255,0.1) 0%, rgba(255,215,0,0.2) 50%, rgba(255,255,255,0.1) 100%);
            border-radius: 50%;
    ...
    @foreach($pages as $pageIndex => $page)
        <div class="page">
            <div class="page-header">
                <h2>{{ $foundationName }} - ID Card Print Batch</h2>
                <p>Batch: {{ $batch->batch_name }} | Page {{ $pageIndex + 1 }} of {{ count($pages) }}</p>
            </div>
            <table class="cards-table">
                @foreach(array_chunk($page['cards'], 2) as $row)
                    <tr>
                        @foreach($row as $card)
                            <td>
                                <div class="card-wrapper">
                                    @if($page['showCutMarks'])
                                    ...
                                    @endif
                                    @include('id-cards.card-content', $card)
                                </div>
                            </td>
                        @endforeach
                        @if(count($row) == 1)
            ...
            <div class="print-instructions">
                Print on CR80 PVC card stock (85.60mm x 53.98mm) using a card printer. Cut along dashed lines if using sheet stock.
            </div>
        </div>
    @endforeach
</body>
